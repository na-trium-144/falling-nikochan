...
import { app, createTestPlaySession, dummyCid } from "./init.js";
...
describe("POST /api/playSession/sign", () => {
  test("should sign play result with ResultSecret key", async () => {
    ...
    const res = await app.request("/api/playSession/sign", {
      method: "POST",
      headers: {
  ...
  test("should return 422 when cid is different", async () => {
    const { sessionToken, sessionKeyPair } = await createTestPlaySession(
