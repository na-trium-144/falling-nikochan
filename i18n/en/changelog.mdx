...
import { Hidden } from "@/common/mdxHiddenPart.js";

## ver. 17.10 - 09/20/2026

- Since a bug affecting multi-touch input (two or more points) was fixed in iOS 27, the feature that predict for potential unresponsiveness has been disabled, and tap sounds are now enabled by default.

## ver. 17.8 - 09/18/2026

- Fixed the screen edge and Safari toolbar background appearing white on iOS.
- Fixed an issue where the Exit button did not respond when opened in the in-app browser of X (Twitter).
- Adjusted the gameplay screen layout for smaller screen sizes.

## ver. 17.5 - 09/03/2026

- Fixed an issue where levels would duplicate when reordering in the chart editor.
- Fixed an error that occurred when the chart's start position was set after its end position.
- Added an error message display for cases where the parameter in a shared play result URL is incorrect.

<Hidden>

## ver. 17.4 - 08/26/2026

- Fixed an issue where symbols such as "☆" were not displaying correctly in preview images for shared charts and play records.
- Fixed a display misalignment for certain icons on iOS 26.
- Disabled editing for charts saved without a password in ver.8 or earlier.

## ver. 17.0 - 08/11/2026

- Fixed an issue where, when the chart speed was set to a negative value, notes configured to fall "from top" would sometimes move downwards before returning to the top.
  - Charts saved to the server prior to ver. 16 will continue to display using the previous behavior; the new behavior applies only to charts overwritten and saved from this version onwards.
- Replaced the slime image. (<img src={process.env.ASSET_PREFIX + "/assets/slime-closed.svg"} className="inline-block w-6 "/> → <img src={process.env.ASSET_PREFIX + "/assets/slime-2-closed.svg"} className="inline-block w-6 "/>)
- Added an animation where the slime in the bottom-right corner hops around when the results screen is displayed.
- Standardized the appearance of notes in the chart editor to match gameplay (using the note images introduced in ver. 14).
- Reverted the note effects in gameplay to the style used in ver. 9.3 and earlier.

## ver. 16.30 - 07/27/2026

- Added a display of the levels to the preview image when sharing charts with URL.

## ver. 16.25 - 07/13/2026

- Added the invite link for the official Discord server to the top page.

## ver. 16.23 - 07/10/2026

- Added "Undo" and "Redo" buttons to the chart editor.
- Fixed the display position of "Running script..." and error messages to the bottom-right corner.

## ver. 16.19 - 06/14/2026

- Added a search box to the top page.
- When a chart ID is entered in the search box, the information for that chart will be displayed without waiting for search results.

## ver. 16.17 - 06/13/2026

- An error will now be displayed if the chart has been updated between selecting a song and opening the play screen.
- When deleting a chart, it will now immediately disappear from the list of recently edited charts without requiring a page refresh.

## ver. 16.5 - 05/21/2026

- The "Recently Played Charts" list is now accessible again integrated with the search function.

## ver. 16.4 - 05/17/2026

- The input fields in the timing and note tabs of chart editor now supports math expressions.

## ver. 16.0 - 05/15/2026

- The title page design has been revamped.

## ver. 15.10 - 05/14/2026

- Updated Terms of Use.
  - Added information regarding copyright for chart data and disclaimer.
  - Clarified that CC-BY-4.0 copyright notice is not required when publishing gameplay videos.
  - Added information regarding data collected by Cloudflare Analytics and Sentry.

## ver. 15.7 - 04/18/2026

- The time bar in the chart editor can now be scrolled by dragging.
- Notes on the time bar in the chart editor can now be selected by clicking.

## ver. 15.5 - 04/15/2026

- The "Popular Chart" and "Latest Chart" lists have been integrated into the search function.
  - Search results can now be sorted by newest first or by number of plays.
  - A function to filter by difficulty level has been added.
  - The buttons for each sheet music are now larger, and the difficulty level is displayed (PC view only).

## ver. 15.3 - 04/02/2026

- Falling Nikochan website URL in an iframe would no longer cause an error.
- Added an RSS feed for new charts.

## ver. 15.0 - 03/25/2026

- Changed the file format for saving score chart locally (~.fn.lua)
  - Saved Lua files can be read using the [fn-commands](https://github.com/na-trium-144/fn-commands) library.
  - Files in the previous format (~.fn14.yml, etc.) can still be read.
- Improved the error messages when reading local files in chart editor.
- Improved the explanation of data formats in the [API Documentation](/api).
- Added /api/seqFile and /api/seqPreview APIs.

## ver. 14.22 - 02/19/2026

- Saving a chart with the "Save password" option unchecked now clears the saved password for that chart.
- The "Please wait 3 seconds and try again" message no longer appears in chart editor, and it will automatically retry multiple times.

## ver. 14.18 - 02/18/2026

- The time bar in the chart editor can now be scrolled horizontally.
- Fixed an issue where moving the cursor immediately after opening the score editor would start the video playback.

## ver. 14.15 - 02/13/2026

- Adjustments have been made to some text, such as titles and headings, by increasing the contrast and making it bold.

## ver. 14.13 - 02/12/2026

- Fixed an issue where you could not add the second or subsequent note using the Add button on the note tab of the chart editor.

## ver. 14.12 - 02/09/2026

- Fixed an issue where the cursor would reset each time a note was edited when multiple notes were placed at the same time.

## ver. 14.7 - 02/04/2026

- Added a contact link and a back-to-top button to the server error page.
- Implemented an error message display for client errors.

## ver. 14.6 - 02/03/2026

- Fixed an issue where the chart editing page was slower than in ver.13.

## ver. 14.0 - 02/01/2026

- The site design has been revamped.
- Added a feature that automatically adjusts the chart offset in real time during play.
  - You can disable this in the options and continue to set it manually as before.
- The frame rate setting has been removed; now it's automatically adjusted.
- The copy/share link buttons have been redesigned so that the copy button only copies the URL.
  - On PC, you can copy both the text and URL from the "Share" menu as before, or post directly to <XLogo />.
  - On Android/iOS, pressing the "Share" button will display the OS's share menu as before, allowing you to send to other apps.
- Game screen design changes
  - Notes are now semi-transparent, and a trail has been added.
  - The judge line lights up even in autoplay.
  - Added a score gauge.
  - Score per note is now displayed.
  - Added animations to display the chain count every 50 chains and misses.
  - The UI is larger than ever on PC with larger screen sizes.
- The color of YouTube video borders changes to match the video thumbnail.
- The order of overlapping notes has been reversed on both the play screen and the score editing screen.
- Chart editing function
  - You can now delete notes using the <Key className="text-sm p-0.5 ">Delete</Key> key / <Key className="text-sm p-0.5 ">BackSpace</Key> key.
  - Added the ability to flip notes horizontally. ( <Key className="text-sm p-0.5 ">M</Key> key)
  - The size of one step and the zoom state are now saved with the chart data.
  - The cursor and barline positions are now easier to see in the code tab.
  - Fixed the behavior when selecting a range in the code tab.
- Overwriting a chart from previous versions (ver. 7-13) will no longer erase play history if no changes have been made to the content.
- Starting with this version, the license of the source code of Falling Nikochan has been changed from MIT to AGPL-3.0

## ver. 13.21 - 01/07/2026

- In auto-play mode, notes will always result in a "Good" judgment.
  - Adding `&judgeauto=1` to the URL will restore the previous behavior.

## ver. 13.18 - 12/23/2025

- Fixed the calculation of the appearance position when notes appear during chart speed acceleration/deceleration.
...
- An official <XLogo /> (Twitter) account [@nikochan144](https://twitter.com/nikochan144) has been launched. It will automatically tweet when a new chart is published or updated.
...
- Added an image of <img src={process.env.ASSET_PREFIX + "/assets/slime-closed.svg"} className="inline-block w-6 " /> in addition to <img src={process.env.ASSET_PREFIX + "/assets/slime.svg"} className="inline-block w-6 " /> , and made it animate.
