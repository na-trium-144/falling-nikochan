...
import { MoveIcon } from "@/common/mdxIcons.js";
...
you can change them with the mouse from the note preview displayed on the left.
  ...
  - For touch operation, press the <MoveIcon /> Touch mode
  button to switch to a mode in which you can move the note's position/speed by touch operation.
- You can delete the note at the current cursor position using the <Key className="text-sm p-0.5 mx-0.5">Delete</Key> key or <Key className="text-sm p-0.5 mx-0.5">BackSpace</Key> key.
- You can flip the note at the current cursor position horizontally using the <Key className="text-sm p-0.5 mx-0.5">M</Key> key.
- You can undo edits using the Undo/Redo buttons or the <Key className="text-sm p-0.5 mx-0.5">Z</Key> / <Key className="text-sm p-0.5 mx-0.5">Y</Key> keys. 
  - This applies not only to note editing but also to changes made in the Timing and Code tabs.
  - Edit history is recorded separately for each level.
