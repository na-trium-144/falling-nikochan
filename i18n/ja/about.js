export default {
  about: {
    ...
    2: {
      ...
      contentIOS:
        "iOSでは、ブラウザー(Safari以外も含む)のバグにより2本以上の指でのタップが無反応になる場合があります。" +
        " Falling Nikochan では特定の条件でこれを予測して補正し、" +
        "タップが反応せずスルーした場合にミス判定にせずコンボを継続する処理を入れています。" +
        "(これはデフォルトで有効ですがオフにもできます)",
      contentIOS2:
        "この補正は100%正確ではなく、一部音符がミス判定になってしまったり、稀にタップしていない音符が反応してしまうこともあります。",
      contentIOS3:
        "またタップ音は補正されないので、無反応補正が有効の場合タップ音はオフにしています。",
      chain: "コンボ",
      description:
        "ニコチャンが線に重なったときに音符を叩くだけの簡単なルールです。" +
        "ニコチャンの位置や飛び方によらず、" +
