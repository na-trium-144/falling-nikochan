...
export default function TimeBar(props: Props) {
  ...
  // timebar左端の時刻
  const timeBarBeginSec = -timeBarWidth / 2 / zoomPxPerSec();
  ...
  const timeBarBeginStep =
    chart && currentLevel && cur
      ? getStep(
          ...
          cur.timeSec - chart.offset - timeBarWidth / 2 / zoomPxPerSec(),
  ...
  if (currentLevel && cur) {
    ...
    while (true) {
      ...
      if (t - cur.timeSec < timeBarWidth / 2 / zoomPxPerSec()) {
  ...
  return (
    <div className="min-w-0 w-full overflow-auto" ref={timeBarRef}>
      <div
        className={clsx(
          "relative mt-10 mb-20 h-6 bg-gray-500/25 overflow-visible"
        )}
        style={{
          marginLeft: timeBarWidth / 2,
          marginRight: timeBarWidth / 2,
          width: (currentLevel?.meta.ytEndSec ?? 0) * zoomPxPerSec(),
        }}
      >
        {/* 秒数目盛り */}
        {Array.from(
          ...
          <Fragment key={dt}>
            <span
              ...
              style={{
                top: -1.25 * rem,
                bottom: -4,
                left: timeBarPos((cur?.timeSec ?? 0) + dt),
              }}
            >
              {timeSecStr((cur?.timeSec ?? 0) + dt)}
            </span>
            <span
              className="absolute border-l border-gray-500 "
              style={{
        ...
        {chart &&
          currentLevel?.seqNotes.map((n) => (
            ...
            // 同じ位置に2つ以上の音符を重ねない
            n.hitTimeSec !==
              currentLevel?.seqNotes.at(n.id + 1)?.hitTimeSec &&*/ <span
              key={n.id}
              className={clsx(
