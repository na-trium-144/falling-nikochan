...
describe("resultParams", () => {
  ...
  describe("parseResultParams", () => {
    ...
    test("should decompress result", async () => {
      const dummyResult = new Uint8Array([0x92, 2, 3]).buffer;
      const dummySign = new Uint8Array([4, 5, 6]).buffer;
      const param =
        (await compressResultParam(encodeBase64Url(dummyResult))) +
        "." +
        (await compressResultParam(encodeBase64Url(dummySign)));
      const { result, sign } = await parseResultParams(param);
      expect(encodeBase64Url(result.buffer).replaceAll("=", "")).to.be.equal(
        encodeBase64Url(dummyResult).replaceAll("=", "") // decompressed
      );
      expect(
        encodeBase64Url(result.buffer).replaceAll("=", "")
      ).not.to.be.equal(
        (await compressResultParam(encodeBase64Url(dummyResult))).replaceAll(
          "=",
          ""
        )
      );
      expect(encodeBase64Url(sign!.buffer).replaceAll("=", "")).to.be.equal(
        (await compressResultParam(encodeBase64Url(dummySign))).replaceAll(
          "=",
          ""
        )
      );
    });
    test("should ignore unknown characters", async () => {
      const dummyResult = new Uint8Array([0x92, 2, 3]).buffer;
      const dummySign = new Uint8Array([4, 5, 6]).buffer;
      const param =
        encodeBase64Url(dummyResult) +
