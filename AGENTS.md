...
- clsxは `"clsx"` ではなく `"clsx/lite"` からimportします。
...
- **譜面ロジックとバックエンドのテスト実行**: `pnpm run test` (MongoDBが起動しており、環境変数`MONGODB_URI`がセットされている必要があります)

### 変更を加えたあと(PR作成前)のチェック

- フォーマッターを実行してコードスタイルを統一してください。
- 型チェックとリンターを実行してエラーがないことを確認してください。 (warningは修正必須ではありません)
- (MongoDBが実行可能な環境の場合) テストを実行してすべてのテストがパスすることを確認してください。ただし chart/ または route/ 以下のファイルに変更を加えていない場合はテストの動作確認は必要ありません。
